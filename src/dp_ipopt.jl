...
function DoublePendulumMOI(model::DoublePendulum, params::SimParams,
    ...
    DoublePendulumMOI{p}(model, params, Qr, Qq, R, r0, q0, Xref,
        n_nlp, m_nlp, N, L, p, ncons, rinds, qinds, xinds, uinds, λinds
    )
end
...
function MOI.eval_constraint_jacobian(prob::DoublePendulumMOI, jac, x)
...
function ipopt_solve(prob::DoublePendulumMOI, x0; tol=1e-6, c_tol=1e-6, max_iter=1_000)
