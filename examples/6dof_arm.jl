...
# Visualize the goal as a green sphere
...
@test sparse(row, col, jvec) ≈ jac  # make sure the constraint Jacobian is right

#############################################
## Solve w/ Ipopt
#############################################
eeonly = true   # only penalize end-effector location and velocities
Qr = Diagonal(SA_F64[1,1,1.]) * !eeonly
Qq = Diagonal(SA_F64[1,1,1,1.]) * !eeonly
Qν = Diagonal(@SVector fill(0.1, 3)) * eeonly
Qω = Diagonal(@SVector fill(0.1, 3)) * eeonly
...
# * zero controls and joint forces (valid since there's not gravity)
z0 = zeros(prob.n_nlp)
...
p_torques = @pgf TikzPicture(
    Axis({
        ...
        "legend style"="{at={(0.93,0.93)},anchor=north east, legend columns=3}",
    },
        torqueplots...,
